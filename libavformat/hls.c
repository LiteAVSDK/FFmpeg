...
struct segment {
    ...
    //保存遇到discontinuity分隔符后的第一个pts，用于后续pts的计算
    //pts = 当前discontinuity分隔符之前的总时长 + discontinuity后当前pts与discontinuity_first_pts的差值
    int64_t discontinuity_first_pts;
    int64_t duration;
    int64_t start_time;
    int64_t url_offset;
    int64_t size;
...
struct playlist {
    ...
    //用于计算seek返回的pts和seek_timestamp的offset，解决误差引起seek一直往后跳进度的问题
    int64_t seek_pts_offset;
    ...
    while (!avio_feof(in)) {
        ...
        if (av_strstart(line, "#EXT-X-STREAM-INF:", &ptr)) {
            ...
            if (is_segment) {
                ...
                //初始化置为AV_NOPTS_VALUE，表示未被赋值与使用过
                seg->discontinuity_first_pts = AV_NOPTS_VALUE;
...
static void aes_decrypt_key(HLSContext *c, struct playlist *pls) {
    do {
...
static int open_input(HLSContext *c, struct playlist *pls, struct segment *seg)
{
    ...
    av_log(pls->parent, AV_LOG_VERBOSE, "HLS request for url '%s', offset %"PRId64", playlist %d, seg key_type %d\n",
           seg->url, seg->url_offset, pls->index, seg->key_type);
        ...
        if (pls->needed && !pls->pkt.data) {
            while (1) {
                ...
                if (pls->seek_stream_index < 0 ||
                    ...
                    //seek第一次进来时候，做一下offset的校验
                    int64_t start_time = s->start_time > 0 ? s->start_time : 0;
                    //恢复为9.1版本之前一样的判断条件，符合落在当前分片预期直接返回
                    ts_diff = start_time + timestamp + av_rescale_rnd(pkt_ts, AV_TIME_BASE,
                            tb.den, AV_ROUND_DOWN) -
                            pls->seek_timestamp;
                    //第0个分片直接返回
                    if(timestamp == 0 && pls->cur_seq_no == 0 && ts_diff < 0){
                        ts_diff = 0;
                    }

                    //AVSEEK_FLAG_ANY返回准确位置
                    if (ts_diff >= 0 && (pls->seek_flags & AVSEEK_FLAG_ANY)) {
                        pls->seek_timestamp = AV_NOPTS_VALUE;
                        break;
                    }

                    //AV_PKT_FLAG_KEY返回seek位置前的关键帧
                    if (ts_diff >= 0 && (pls->pkt.flags & AV_PKT_FLAG_KEY)) {
                        pls->seek_timestamp = AV_NOPTS_VALUE;
                        //seek操作发生的时候，根据之前的累计时长估算出当前的discontinuity_first_pts：
                        if (pls->cur_seq_no > 0 && pls->cur_seq_no < pls->n_segments && pls->pkt.stream_index >= 0 && pls->pkt.stream_index < pls->n_main_streams && s->streams[pls->pkt.stream_index] && pls->segments[pls->cur_seq_no]->previous_duration > 0) {
                            int64_t discontinuity_diff = av_rescale_q(pls->segments[pls->cur_seq_no]->start_time - pls->segments[pls->cur_seq_no]->previous_duration,
                                                                      AV_TIME_BASE_Q,
                                                                      s->streams[pls->pkt.stream_index]->time_base);
                            pls->segments[pls->cur_seq_no]->discontinuity_first_pts = pkt_ts - discontinuity_diff;
                        }
                        break;
                    }
                }
        ...
        if (c->playlists[minplaylist]->finished) {
            ...
            if (seq_no < pls->n_segments && s->streams[pkt->stream_index]) {
                ...
                int64_t max_ts = av_rescale_q(seg->start_time + seg->duration,
                                              AV_TIME_BASE_Q,
                                              s->streams[pkt->stream_index]->time_base);
                int64_t ts_start_time = av_rescale_q(seg->start_time,
                                              AV_TIME_BASE_Q,
                                              s->streams[pkt->stream_index]->time_base);
                ...
                //read_packet过程中，不断的给discontinuity_first_pts赋值
                if(seq_no > 0 && seq_no < pls->n_segments && pls->segments[seq_no]->previous_duration > 0 && pls->segments[seq_no]->discontinuity_first_pts == AV_NOPTS_VALUE) {
                    if (pls->segments[seq_no]->previous_duration > pls->segments[seq_no-1]->previous_duration) {
                        //表示discontinuity分隔符的前后2个TS

                        //赋值保护，校正后的预期值为：
                        //pkt->pts = pred + pkt->pts - pls->segments[seq_no]->discontinuity_first_pts;
                        //如果给pls->segments[seq_no]->discontinuity_first_pts赋值为pkt->pts，则需要判断赋值后的pts是否符合预期
                        if(pkt->pts >= ts_start_time && pkt->pts <= max_ts){
                            //预期校正后的值符合预期，才给discontinuity_first_pts赋值
                            pls->segments[seq_no]->discontinuity_first_pts = pkt->pts;
                        }
                    } else if (pls->segments[seq_no]->previous_duration < pls->segments[seq_no]->start_time &&
                        pls->segments[seq_no-1]->discontinuity_first_pts != AV_NOPTS_VALUE) {
                        //表示同一个discontinuity分隔符后的连续TS
                        pls->segments[seq_no]->discontinuity_first_pts = pls->segments[seq_no-1]->discontinuity_first_pts;
                    }
                }

                if((pkt->pts < ts_start_time || pkt->pts > max_ts) && seq_no > 0 && seq_no < pls->n_segments && pls->segments[seq_no]->discontinuity_first_pts != AV_NOPTS_VALUE){
                    //有discontinuity标签的pts做校正，discontinuity_first_pts已做过赋值保护
                    pkt->pts = pred + pkt->pts - pls->segments[seq_no]->discontinuity_first_pts;
                }
